# XUI-IM Development Guide

## Project Overview
XUI-IM (xui-im) is a web-based control panel for managing Xray-core servers. It's a Go application using Gin web framework with embedded static assets and SQLite database. The panel manages VPN/proxy inbounds, monitors traffic, and provides Telegram bot integration.
...
### Building & Running
```bash
# Build (creates the panel binary, e.g. bin/xui-im.exe on Windows)
