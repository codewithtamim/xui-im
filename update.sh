...
fetch_latest_version() {
    ...
    while [[ $i -lt $max_retries ]]; do
        ...
        if [[ -n "$token" ]]; then
            response=$(eval "${curl_bin} ${curl_opts} -H \"Authorization: Bearer ${token}\" \"${api_url}\"" 2>/dev/null)
        else
            response=$(eval "${curl_bin} ${curl_opts} \"${api_url}\"" 2>/dev/null)
        fi
